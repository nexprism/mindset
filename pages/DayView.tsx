...
import { updateModuleProgress } from '../services/storage';
import { ArrowLeft, BookOpen, CheckCircle, ChevronLeft, ChevronRight, PenTool, CheckSquare, Languages, Mic, MicOff, Clock, Trophy, ArrowRight, Check, Sparkles, Lock, SkipForward } from 'lucide-react';
...
const parseInline = (text: string): React.ReactNode[] => {
    ...
    return parts.map((part, i) => {
        ...
        // Note: This regex is simple and might catch non-italic asterisks, but suffices for controlled content.
...
const DayView: React.FC<DayViewProps> = ({ userState, onUpdate, onToggleLang }) => {
  ...
  useEffect(() => {
    ...
    // 1. Enforce Sequential Access
    // You cannot jump to day 5 if maxCompleted is 2.
    if (dayNum > maxCompleted + 1) {
       navigate(`/module/${moduleId}`, { replace: true });
       return;
    }

    // 2. Enforce Frequency (1 Activity Per Day)
    // If you are trying to start the next new day (maxCompleted + 1)
    if (dayNum === maxCompleted + 1 && maxCompleted > 0) {
        ...
        if (lastEntry) {
            ...
            // Check if last activity was done today
            if (lastDate.toDateString() === today.toDateString()) {
                // It is locked. Alert and kick back.
                const msg = lang === 'en' 
  ...
  // Effect 1: Navigation Reset (Run only when day/module changes)
  useEffect(() => {
  ...
  // Effect 2: Data Loading (Run when data or day changes)
  useEffect(() => {
    let newJournalText = '';
    let newTaskResponse = '';
    ...
    if (moduleId && userState.progress[moduleId]) {
      ...
      // Load saved entry data if it exists
      if (entry) {
  ...
  // Timer Logic: Reset on step change
  useEffect(() => {
    setTimerSeconds(0);
  }, [currentStep]);

  // Timer Logic: Run timer with visibility check
  useEffect(() => {
    let intervalId: any;

    const tick = () => {
      // Pause timer if tab is hidden to ensure active reading time
      if (document.hidden) return;

      setTimerSeconds(prev => {
        // Cap at 300s (5 mins) for reading
        if (currentStep === 'reading' && prev >= 300) {
           return 300;
        }
        return prev + 1;
      });
  ...
  const handleNextQuestion = () => {
    ...
    // Filter out the currently displayed prompt to ensure variety
    if (activePrompt) {
        availablePrompts = ADDITIONAL_REFLECTION_PROMPTS.filter(
            p => p.en !== activePrompt.en
        );
    }
    
    // Fallback if list is empty (shouldn't happen given constant data)
    if (availablePrompts.length === 0) {
        availablePrompts = ADDITIONAL_REFLECTION_PROMPTS;
    }

    const randomIndex = Math.floor(Math.random() * availablePrompts.length);
    setActivePrompt(availablePrompts[randomIndex]);
    
    // Add spacing for new answer if user has already typed something
    if (journalText && !journalText.endsWith('\n\n')) {
        setJournalText(prev => prev.trim() + '\n\n');
        setHasUnsavedChanges(true);
    }

    // Focus text area for better UX
    setTimeout(() => {
  ...
  const handleMicToggle = (field: 'task' | 'reflection') => {
    ...
    // 1. Stop active recognition if running
    if (recognitionRef.current) {
      // Important: Unbind onend to prevent state clearing logic from running during this manual stop
      recognitionRef.current.onend = null; 
      recognitionRef.current.stop();
      recognitionRef.current = null;
    }

    // 2. If we are toggling the SAME field off, just clear state and return
    if (activeMicField === field) {
      setActiveMicField(null);
      return;
    }

    // 3. If starting a NEW field (or switching), add a slight delay to ensure browser releases audio resource
    // This prevents "service-not-allowed" errors when switching rapidly
    setTimeout(() => {
        ...
        recognition.onerror = (event: any) => {
          ...
          if (event.error === 'not-allowed') {
             alert("Microphone access blocked. Please allow permission in your browser settings.");
          }
          setActiveMicField(null);
          recognitionRef.current = null;
        };
        ...
        recognition.onresult = (event: any) => {
          ...
          if (cleanTranscript) {
             const updateText = (prev: string) => {
                 ...
                 // Add space if needed (if prev exists and doesn't end in whitespace)
                 const needsSpace = prev.length > 0 && !/\s$/.test(prev);
                 
                 // Auto-capitalize if starting new sentence (prev is empty or ends in .!?)
                 const isStartOfSentence = prev.trim().length === 0 || /[.!?]$/.test(prev.trim());
                 
                 if (isStartOfSentence && textToAppend.length > 0) {
             ...
             if (field === 'task') {
                setTaskResponse(prev => updateText(prev));
             } else {
                setJournalText(prev => updateText(prev));
             }
             setHasUnsavedChanges(true);
          }
        };
    ...
    }, 150); // 150ms delay to prevent state update race conditions
  };
  ...
  // Calculate Next Day Lock Status for UI
  const maxCompleted = progress?.completedDays.length > 0 ? Math.max(...progress.completedDays) : 0;
  let isNextDayLocked = false;
  // If we are currently viewing the latest completed day (or a previous one, but logically we care about the frontier)
  // Actually, we specifically care if the user tries to go to 'maxCompleted + 1'
  // If we are on 'maxCompleted', and we want to show 'Next Day' button...
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 'reading':
        const readingDuration = 300; // 5 minutes required for all days
        ...
        return (
          <section className="animate-fade-in pb-32">
            <div className={`bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl shadow-sm border mb-6 transition-all duration-300 ${readingConfirmed ? 'border-green-400 ring-1 ring-green-50 dark:ring-green-900' : 'border-slate-100 dark:border-slate-800'}`}>
              ...
              <div className="flex justify-end mb-6">
                ...
                >
                    ...
                    <span>{lang === 'en' ? 'Translate to Hindi (हिंदी)' : 'Translate to English'}</span>
                </button>
              </div>

              {/* Enhanced Content Rendering */}
              <ContentRenderer content={dayContent.reading[lang]} />
            ...
            {/* Sticky Progress Bar / Action Footer */}
            {!isCompleted && (
                <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-6 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] z-50 animate-slide-up">
                        ...
                        {!readingConfirmed ? (
                             timerSeconds < readingDuration ? (
                                 <>
                                    ...
                                    <div className="flex justify-between items-center mt-3">
                                        <p className="text-xs text-slate-400 font-medium">
                                            {lang === 'en' ? 'Keep reading to unlock the next step' : 'अगले चरण को अनलॉक करने के लिए पढ़ते रहें'}
                                        </p>
                                        <button
                                            onClick={() => {
                                ...
                                <button
                                    ...
                                    className="w-full py-4 bg-orange-600 text-white rounded-xl font-bold shadow-lg hover:bg-orange-700 transition-all animate-bounce-in flex items-center justify-center text-lg transform hover:scale-[1.02]"
                                >
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-green-600 text-white rounded-xl font-bold shadow-lg hover:bg-green-700 transition-all animate-fade-in flex items-center justify-center text-lg transform hover:scale-[1.02]"
                            >
                                <CheckCircle size={24} className="mr-3" />
            ...
            {/* Static Navigation (Visible only when revisiting a completed day) */}
            {isCompleted && (
                <button
                onClick={() => {
                    setCurrentStep('task');
                ...
                className="w-full py-4 rounded-xl font-bold text-lg shadow-lg flex items-center justify-center transition-colors bg-orange-600 text-white hover:bg-orange-700"
                >
        ...
        return (
          <section className="animate-fade-in">
            <div className="bg-indigo-50 dark:bg-indigo-950 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-900 mb-6">
              ...
              <p className="text-indigo-800 dark:text-indigo-300 mb-6 font-medium text-lg leading-relaxed">
              ...
              {activeMicField === 'task' && (
                 <div className="mt-2 text-xs font-bold text-red-500 flex items-center animate-pulse">
            ...
            <div className="flex space-x-4">
              ...
              <button
                ...
                className="flex-1 py-4 bg-orange-600 text-white rounded-xl font-bold shadow-lg hover:bg-orange-700 transition-colors flex items-center justify-center"
              >
        ...
        return (
          <section className="animate-fade-in">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-6">
              ...
              <p key={activePrompt ? activePrompt.en : 'default'} className="text-slate-600 dark:text-slate-300 italic mb-6 text-lg animate-fade-in">{activePrompt ? activePrompt[lang] : dayContent.reflectionPrompt[lang]}</p>
            ...
            <div className="flex flex-col space-y-4 mb-8">
                <div className="flex space-x-4">
                  ...
                  <button
                    ...
                    className={`flex-1 py-4 rounded-xl font-bold shadow-lg transition-colors flex items-center justify-center ${
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-300">
      {/* Top Nav */}
      <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 p-4 sticky top-0 z-40 shadow-sm">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <Link to={`/module/${moduleId}`} className="text-slate-500 dark:text-slate-400 hover:text-orange-600 dark:hover:text-orange-400 transition-colors">
            <ArrowLeft size={24} />
          </Link>
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-6">
        ...
        {/* Prominent Next Action Navigation for Completed Days */}
        {isCompleted && (
           <div className="mt-8 pt-8 border-t border-slate-200 dark:border-slate-800 w-full animate-fade-in">
             <div className="flex flex-col space-y-4">
                 {isNextDayLocked ? (
                     <div className="w-full py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-xl font-bold text-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center cursor-not-allowed">
                         <Lock size={20} className="mr-3" />
                         <span>{lang === 'en' ? 'Next Session Unlocks Tomorrow' : 'अगला सत्र कल खुलेगा'}</span>
                     </div>
                 ) : nextDay ? (
                   <Link 
                     to={`/module/${moduleId}/day/${nextDay}`} 
                     className="w-full py-4 bg-orange-600 text-white rounded-xl font-bold text-lg shadow-lg hover:bg-orange-700 transition-all flex items-center justify-center group"
                   >
                    ...
                    <Link 
                      ...
                      className="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold text-lg shadow-lg hover:bg-emerald-700 transition-all flex items-center justify-center group"
                    >
