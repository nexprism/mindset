...
import { Lock, ImageOff, Star, ArrowRight, Flame, Clock, Calendar, Trophy, List, Quote, Sparkles } from 'lucide-react';
...
const Home: React.FC<HomeProps> = ({ userState }) => {
  ...
  // Identify Active Journey
  // A journey is active if > 0 days completed AND not fully completed
  const activeJourneyEntry = Object.entries(userState.progress).find((entry) => {
  ...
  const activeJourneyId = activeJourneyEntry ? activeJourneyEntry[0] : null;
  const activeJourneyModule = activeJourneyId ? MODULES.find(m => m.id === activeJourneyId) : null;

  const handleLockedClick = () => {
     const titleEn = activeJourneyModule?.title.en || 'current';
     const titleHi = activeJourneyModule?.title.hi || 'वर्तमान';

     const msg = lang === 'en'
        ? `It will open after completing the current journal: "${titleEn}".`
        : `यह वर्तमान जर्नल पूरा करने के बाद खुलेगा: "${titleHi}".`;
     alert(msg);
  };
        ...
        {recommendedModule && (
          <section>
            ...
            <ModuleCard 
              ...
              userState={userState} 
              isHighlight={true}
              isLocked={!!activeJourneyId && activeJourneyId !== recommendedModule.id}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {otherModules.map(m => (
              <ModuleCard 
                ...
                module={m} 
                userState={userState}
                isLocked={!!activeJourneyId && activeJourneyId !== m.id}
                onLockedClick={handleLockedClick}
              />
            ))}
          </div>
